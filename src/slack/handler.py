...
def run_betterbrain_cascade(question: str, betterbrain_config) -> Tuple[Optional[str], str]:
    ...
    answer, status = _run_claude(
        ...
        "   Step 1: read the record's `product_area` FIELD. It is a controlled value from "
        "taxonomy.json, not prose -- Admin, AI, Analytics, API, Calibration, Conversations, "
        "Engage, Feedback, Goals, Integrations, Meetings, Permissions, Platform, "
        "Recognition, Reporting, Reviews. Use the field, not your reading of the text.\n"
        "   Step 2: name the product area the QUESTION is about. If it does not match the "
        "record's `product_area`, that record CANNOT carry the lead answer -- full stop, "
        "however well its content seems to fit. Cite a record from the right area, or say "
        "the right area has no coverage.\n"
        "   NEVER reconcile a mismatch by inventing a combined product name. If the record "
        "says one area and the question is about another, they are DIFFERENT THINGS; you "
        "may not merge them into a surface that makes the mismatch disappear. Broken on "
        "2026-07-22: an Engage question was answered by citing a product_area Admin record "
        "as `Verified`, under an invented product name that merged the two areas. That "
        "name exists nowhere in the corpus; it was constructed to make the citation "
        "defensible, and the correct same-area record went uncited. Every other check "
        "passed -- real citation, accurate numbers transcribed from the wrong record, "
        "sound arithmetic -- because only the product identity was fake. That is what "
        "makes this failure mode dangerous: nothing on the surface looks wrong.\n"
        "   Step 3: then read the `fact` field and list any PROVIDER (BambooHR, UKG, "
        "Workday, TriNet Zenefits), EDITION (Classic, NextGen) or SURFACE (anytime vs "
        "scheduled, mobile, API, kiosk) that it names.\n"
        "   Step 4: if it names one the QUESTION did not, that record CANNOT be the "
